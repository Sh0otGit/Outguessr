...
**Status: fully live — the whole loop is real, admin panel included.** `POST /api/submit`, `GET /api/results/:day`, and `GET /api/count/:day` are deployed and verified against the real D1 database at outguessr.com. The 00:03 UTC cron does the full tally job: bot blending, per-format results blob (with a percentile lookup so any pick can be scored without recomputing), Split or Steal pairing, and roast-copy templating, all verified deterministic. The client now consumes all of it — locking in records a pending pick, and payoff (points, tier, ceremony) happens once the day actually closes, reading real `GET /api/results/:day` data instead of simulated `challenges.json` distributions. See the Phase 1 bullets above for the full payoff-then-hook / History flow. `/api/admin/*` is live too — Dashboard, Bots, and System all read/write real data now (see ADMIN-PANEL-PLAN.md's Build status). Not yet implemented: Players section, Cloudflare Access in front of `/admin` (the `X-Admin-Key` check is the only gate right now — see ADMIN-PANEL-PLAN.md's Build order note). See DEPLOY.md for the deploy history and current live configuration.
