...
**Status: fully live — the whole loop is real, admin panel included.** `POST /api/submit`, `GET /api/results/:day`, `GET /api/count/:day`, and `GET /api/challenge/:day` are deployed and verified against the real D1 database at outguessr.com. The 00:03 UTC cron does the full tally job: bot blending, per-format results blob (with a percentile lookup so any pick can be scored without recomputing), Split or Steal pairing, and roast-copy templating, all verified deterministic. The client now consumes all of it — locking in records a pending pick, and payoff (points, tier, ceremony) happens once the day actually closes, reading real `GET /api/results/:day` data instead of a bulk challenges file. See the Phase 1 bullets above for the full payoff-then-hook / History flow. `/api/admin/*` is live too — Dashboard, Bots, and System all read/write real data now, including a full force-close/reopen/reset state machine (see ADMIN-PANEL-PLAN.md's Build status). Not yet implemented: Players section, Cloudflare Access in front of `/admin` (the `X-Admin-Key` check is the only gate right now — see ADMIN-PANEL-PLAN.md's Build order note). See DEPLOY.md for the deploy history and current live configuration.
...
`GET /api/count/:day` was originally a raw `answers` count. It's now `real + projected bots`, ramped smoothly across the UTC day instead of jumping straight to the full bot floor the instant the day opens: `projected = round(botTarget × dayFraction)`, where `botTarget = max(0, bot_floor − real)` and `dayFraction` is how much of the UTC day has elapsed (0 at 00:00 UTC, 1 by 00:00 the next day). A day that's already been tallied just returns the real final `players` figure from its `results` blob instead of projecting — the projection is only ever a stand-in until the real number exists. The public endpoint **never** reveals the real/bot split, only the combined total, per golden rule 2's spirit (counts are fine, shapes aren't — and "how much of this count is fake" is adjacent to shape). The admin gets the honest breakdown via the key-gated `GET /api/admin/count/:day` → `{count, real, bots}`.
